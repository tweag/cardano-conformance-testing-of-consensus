...
4. Create an independent utility that does the following:
  ...
  b. Act as one or more peers serving points as defined on the schedule. Such peers
would instantiate appropriate protocols to serve as upstream peers to the node
under test (NUT).
  c. Open a timing socket to allow the node under test to control the “ticking” of the
schedule.
  d. Shrink and restart test cases upon signal from the node under test.
  e. Re-export shrunk test cases to disk — failing test cases that have been shrunk are
